...
class ImageResolver {
  ...
  static String? _midBannerAssetUrl(String filename) {
    if (!filename.startsWith('mid_banner_')) return null;

    final assetName = filename.substring('mid_banner_'.length);
    if (assetName.isEmpty) return null;
    final assetPath = 'assets/mid-banner-products/$assetName';

    if (kIsWeb) return _webAssetUrl(assetPath);
    return '${_productionHost()}/assets/assets/mid-banner-products/$assetName';
  }
