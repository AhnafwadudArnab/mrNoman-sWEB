...
// Pattern 5: /public/assets/mid-banner-products/filename (new mid-banner route)
if (!empty($segments[0]) && $segments[0] === 'public'
    && !empty($segments[1]) && $segments[1] === 'assets'
    && !empty($segments[2]) && $segments[2] === 'mid-banner-products'
    && !empty($segments[3])) {
    $filename = basename($segments[3]);
    // Check in backend/public/assets/mid-banner-products/
    $assetPath = __DIR__ . '/assets/mid-banner-products/' . $filename;
    if (is_file($assetPath)) {
        serveImage($assetPath);
    }
    // Also check Flutter assets folder
    $altAssetPath = __DIR__ . '/../../assets/mid-banner-products/' . $filename;
    if (is_file($altAssetPath)) {
        serveImage($altAssetPath);
    }
}
